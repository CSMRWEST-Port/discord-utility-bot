...
def check_guild_exists(guild_id: int, connection: psycopg2.extensions.connection):
    with connection.cursor() as cursor:
        cursor.execute("SELECT * FROM guild_preferences WHERE guild_id = %s", (str(guild_id),))
        return cursor.fetchone() is not None


def make_guild_if_not_exists(guild_id: int, connection: psycopg2.extensions.connection):
    with connection.cursor() as cursor:
...
def set_welcome_channel(guild_id: int, channel_id: int, connection: psycopg2.extensions.connection):
...
def set_custom_welcome_message(guild_id: int, welcome_message: str, connection: psycopg2.extensions.connection):
    if (check_guild_exists(guild_id, connection) == False):
        make_guild_if_not_exists(guild_id, connection)
    with connection.cursor() as cursor:
...
def set_goodbye_channel(guild_id: int, channel_id: int, connection: psycopg2.extensions.connection):
    if (check_guild_exists(guild_id, connection) == False):
        make_guild_if_not_exists(guild_id, connection)
    with connection.cursor() as cursor:
...
def set_custom_goodbye_message(guild_id: int, goodbye_message: str, connection: psycopg2.extensions.connection):
    if (check_guild_exists(guild_id, connection) == False):
        make_guild_if_not_exists(guild_id, connection)
    with connection.cursor() as cursor:
